...
type GroupedDocs = Record<string, ApiDoc[]>
  ...
  return (
    <div className="space-y-4">
      {Object.entries(groupedDocs).map(([category, docs]) => (
        <div key={category}>
          <h4 className="capitalize font-medium mb-2 text-muted-foreground">{category}</h4>
          <ul className="list-none p-0 space-y-2">
            {docs.map((doc) => (
              <li key={doc.id}>
                <a href={`/api/${doc.id}`} className={cn('block hover:underline text-sm', currentPath === `/api/${doc.id}` && 'font-bold')}>
                  {doc.title}
                </a>
              </li>
            ))}
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        ...
        <SheetContent side="left" className="w-[280px]">
          ...
          <ScrollArea className="h-[calc(100vh-120px)] mt-4">
            <div className="pr-4">
              <SideNavContent groupedDocs={groupedDocs} currentPath={currentPath} />
            </div>
          </ScrollArea>
        </SheetContent>
      </Sheet>
