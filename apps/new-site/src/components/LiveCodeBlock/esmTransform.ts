...
export async function transformToFunctionBody(code: string) {
  if (esBuildInitializePromise === null) {
    esBuildInitializePromise = new Promise<boolean>((resolve, reject) => {
      esbuild
        .initialize({
          wasmURL:
            "https://cdn.jsdelivr.net/npm/esbuild-wasm@0.27.0/esbuild.wasm",
        })
        .then(() => {
          resolve(true);
        })
  ...
  try {
    ...
    const packages = [...imports, ...startImports]
      .map((match) => match[2])
      .filter((pkg) => pkg !== "react");

    // First convert imports to assignments
    const codeWithAssignments = convertExportsToAssignments(
      convertImportsToAssignments(code),
    );
