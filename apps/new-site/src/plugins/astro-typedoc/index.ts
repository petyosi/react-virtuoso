...
type FrontmatterObject = Record<string, string | number>;
...
const onRendererPageEnd =
  (frontmatterObject?: FrontmatterObject) => (event: PageEvent<any>) => {
    ...
    const group =
      event.model && "comment" in event.model
        ? getGroupFromReflection(event.model as DeclarationReflection)
        : undefined;
...
const fixCrossLinks = (
  content: string,
  fileToGroupMap: Map<string, string>,
): string => {
  // Match markdown links like [Name](path/to/file.md) or [Name](file.md#anchor)
...
const mergeFilesByGroup = async (dir: string): Promise<void> => {
  const grouped: Map<string, GroupedFile[]> = new Map();
  const fileToGroupMap: Map<string, string> = new Map();
  ...
  for (const file of files) {
    ...
    const groupMatch = content.match(/^group:\s*['"]?(\w+)['"]?\s*$/m);
  ...
  for (const [group, groupFiles] of grouped) {
    ...
    const filename = `${orderPrefix}${toKebabCase(group)}.md`;
