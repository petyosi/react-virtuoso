import { Cell, DerivedCell, Signal, changeWith, combine, link, map, pipe, scan, withLatestFrom } from '@virtuoso.dev/gurx'
import { totalCount$ } from './data'
import type { OffsetPoint, SizeRange } from './interfaces'
import { type AANode, newTree, walk } from './sizing/AATree'
        ...
        if (knownSizes.length > 0) {
          ...
          for (let i = 0; i < knownSizes.length; i++) {
            ...
            newSizeTreeState[shortestColumnIndex] = insertRanges(newSizeTreeState[shortestColumnIndex][0], [
              { size, startIndex: indexInTree, endIndex: indexInTree },
            ])
          }

          for (const [tree] of newSizeTreeState) {
