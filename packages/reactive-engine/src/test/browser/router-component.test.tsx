...
describe('RouterComponent', () => {
  ...
  it('renders the active route component with browser history enabled', async () => {
    ...
    const router = createRouter([home$, about$])
  ...
  it('renders without browser history when useBrowserHistory is false', async () => {
    ...
    function TestApp() {
      ...
      return (
        <div>
          <div data-testid="current-route">Route: {currentRoute ?? 'none'}</div>
          <button
            onClick={() => {
              publishUrl('/users/42')
            }}
    ...
    await expect.element(screen.getByText('Home Page')).toBeInTheDocument()
    await expect.element(screen.getByTestId('current-route')).toHaveTextContent('Route: /')
    ...
    await expect.element(screen.getByTestId('current-route')).toHaveTextContent('Route: /users/42')
  ...
  it('syncs route changes to browser URL', async () => {
    ...
    const about$ = Route('/about', () => <div>About Page</div>)
    const user$ = Route('/users/{userId:number}', ({ pathParams }) => <div>User: {pathParams.userId}</div>)
    const router = createRouter([home$, about$, user$])
  ...
  it('handles browser back button navigation', async () => {
    ...
    const router = createRouter([home$, about$, contact$])
    ...
    function TestApp() {
      ...
      return (
        <div>
          ...
          <RouterComponent basePath="/app" router={router} />
        </div>
      )
    }
  ...
  it('renders with layouts when specified', async () => {
    ...
    const home$ = Route('/', () => <div>Home Page</div>)
    const router = createRouter([home$], [rootLayout])
