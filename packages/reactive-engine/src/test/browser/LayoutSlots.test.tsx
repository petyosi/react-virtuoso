...
describe('LayoutSlots', () => {
  ...
  it('renders null when no fill and no default content', () => {
    const slot$ = LayoutSlotPortal()
  ...
  it('clears slot content when fill unmounts', async () => {
    ...
    const screen = render(<TestComponent showFill={true} />)
    await expect.element(screen.getByTestId('fill')).toBeInTheDocument()

    // Unmount the fill
    screen.rerender(<TestComponent showFill={false} />)
  ...
  it('updates slot content when fill children change', () => {
    const slot$ = LayoutSlotPortal()
    ...
    const screen = render(<TestComponent content="Initial content" />)
    const contentEl = screen.getByTestId('content')
    expect(contentEl.element().textContent).toBe('Initial content')

    // Update the content
    screen.rerender(<TestComponent content="Updated content" />)
