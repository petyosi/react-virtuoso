...
The `data` property of the component includes the data it should render and an optional `scrollModifier` object that specifies the location and the scroll behavior to apply when the data changes. For example, when the user submits a message, you append the new message to the existing data set, and scroll to the bottom instantly or with a smooth scroll animation. A different behavior may be applied when messages from other users arrive, or when a chatbot response is updated.
