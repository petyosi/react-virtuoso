...
### Mapping While Preserving an Item Position

Use `data.mapWithAnchor(callbackfn, anchorItemIndex)` when a mapping operation changes item heights and a specific item must stay at the same viewport position. The anchor index refers to the current data before the mapping runs.

```tsx
ref.current.data.mapWithAnchor((message) => (message.id === expandedId ? { ...message, expanded: true } : message), visibleMessageIndex)
```

`mapWithAnchor` has been available since `@virtuoso.dev/message-list` 1.15.0. Use it for same-list item updates. Use `data.replace(..., { anchor })` when records can be inserted, removed, or reordered during a complete replacement.
...
The method accepts `data` and an optional options object:

- `initialLocation` specifies the scroll position after replacement.
- `purgeItemSizes` clears cached item sizes. Use it when the replacement has different item sizing characteristics, such as when switching channels.
- `suppressItemMeasure` prevents the list from measuring items after replacement. Use it only when the existing measurements remain valid.
- `anchor` preserves an existing item's viewport position while records are inserted, removed, or reordered.
...
### Preserving an Item During Replacement

Replacement anchoring is available in `@virtuoso.dev/message-list` 1.18.0 and later. The anchor index is resolved against the data before replacement. Use `'LAST'` for the last item or a negative number to count backward from the end. The list then finds the same item in the replacement data through `itemIdentity` and keeps it at the same pixel position in the viewport.
...
// Keep the current last record stationary while earlier records are spliced in.
ref.current.data.replace(nextMessages, {
  anchor: { index: 'LAST', position: 'preserve' },
})
```

Use a stable `itemIdentity` when replacement data recreates objects. The default identity uses object reference equality, so it cannot match recreated wrappers. If the anchor item is not present in the replacement, the operation continues without anchor compensation.

`anchor` and `initialLocation` describe different replacement policies. Use `anchor` to preserve an item from the current data. Use `initialLocation` to choose a location in an unrelated data set, such as a newly selected channel.
