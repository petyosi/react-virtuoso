...
description: Customize scroll animations with custom easing functions like bounce or elastic for unique chat UX.
...
## Retargeting a Moving Destination

`@virtuoso.dev/message-list` 1.18.0 adds the optional `retarget: true` field to custom smooth-scroll behavior. When another scroll request uses a retargetable behavior while an earlier retargetable animation is active, the list updates the current animation's destination instead of restarting it. The animation preserves its current velocity and continues toward the new target.

Use retargeting for live tails where appends, streaming content, or replacement batches repeatedly move the bottom during an active animation:
...
ref.current.data.append(incomingMessages, ({ atBottom, scrollInProgress }) =>
...
Both the active animation and the new request must set `retarget: true`. Other smooth-scroll behaviors keep their previous behavior and start a new animation for each request. The callback above returns `false` when the user has scrolled away from the live tail.
        ...
        onClick={(e) => {
          ...
          virtuoso.current?.data.append([myMessage], ({ scrollInProgress, atBottom }) => {
            return {
              index: 'LAST',
              align: 'start',
              behavior: atBottom || scrollInProgress ? customSmoothScroll : 'auto',
            }
          })
