...
`SortHeaderButton` is header-slot UI. Always mount it through `HeaderEnd` and render the label as a function. Rendering it directly inside `DataTableColumnHeader` is wrong and can produce a second header strip with raw field names.
...
For application APIs that use a required sort shape such as `{ sortBy, sortOrder }`, prefer an app-level wrapper that owns the slot contract:

```tsx
function SortableDataTableColumnHeader({ field, label, sortBy, sortOrder, onSortChange }) {
  return (
    <DataTableColumnHeader>
      <HeaderEnd>
        {(params) => (
          <SortHeaderButton
            {...params}
            field={field}
            getDirection={() => (sortBy === field ? sortOrder : undefined)}
            getNextDirection={({ direction }) => (direction === 'asc' ? 'desc' : 'asc')}
            getPayload={({ direction }) => ({ sortBy: field, sortOrder: direction ?? 'asc' })}
            onPayloadChange={(payload) => onSortChange(payload)}
          />
        )}
      </HeaderEnd>
      {() => label}
...
Then call sites use the wrapper instead of assembling `HeaderEnd` by hand.
