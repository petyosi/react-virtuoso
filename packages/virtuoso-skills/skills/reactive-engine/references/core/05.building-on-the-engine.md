...
## Computed cells for state projections

Represent a pure function of readable state with `ComputedCell`. This modernized equivalent of data-table's column visibility
projection shows the recommended form for new graphs:

```ts
export const columnVisibilityState$ = ComputedCell(
  [columns$, columnVisibilityOverrides$],
  ([columns, overrides]) => new Map([...columns].map(([key, column]) => [key, overrides.get(key) ?? column.visible !== false]))
)
```

Store only the minimal independent state and derive the rest. `ComputedCell` initializes from current state, including provider seeds,
and the transaction system prevents partially updated inputs. Use `DerivedCell` instead when an arbitrary stream, rather than readable
state, owns the updates.
