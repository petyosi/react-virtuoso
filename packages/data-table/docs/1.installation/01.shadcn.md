...
- Path aliases resolvable from the root config shadcn reads. If a project uses a solution-style root `tsconfig.json` that only contains `references`, mirror the `@/*` path mapping there or shadcn may create a literal `@/components/...` directory instead of writing into `src/components/...`.
