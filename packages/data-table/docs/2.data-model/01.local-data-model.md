...
`localModel()` holds an in-memory row array and exposes actions that mutate or derive rows. The source can be a static array, React state pushed in with `setData`, or a client-side collection updated by user actions.
...
Two things make this more than a glorified `data.filter().sort()`:
...
For the full render-side reference — remote-paginated grouping, sticky-header behavior, and a runnable end-to-end example — see [Grouped Rows](/data-table/features/grouped-rows/).
...
Pair this with `modelStatePersistenceAdapter()` from [State Persistence](/data-table/features/state-persistence/) to save model action state alongside column width, order, and visibility.
