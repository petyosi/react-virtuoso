...
`visible={false}` hides a column by default. Runtime visibility lives in the [Column Visibility](/data-table/features/column-visibility/) feature.

```tsx
<DataTableColumn field="internalSku" visible={false} />
```

`sticky="left"` or `sticky="right"` pins a column for row identity, IDs, or actions that should stay visible during horizontal scrolling — see [Sticky Columns](/data-table/features/sticky-columns/).
