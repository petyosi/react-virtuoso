...
Headers often need more than a label — a sort button, a filter menu, a resize handle, a drop indicator for column reordering. You could compose those inside the header render function, but some of them need layout the render function can't give cleanly: a resize handle has to sit on the column boundary regardless of how the label is aligned, a drop indicator needs to span the entire header during a reorder drag. For these cases, the table provides four named slots:
...
The shadcn wrapper's resize and reorder UI is built on these slots. See [Adding Controls to Column Headers](/data-table/customization/header-slots/) for the placement diagram, the `component` prop alternative to render-function children, and complete recipes for sort, filter, and menu slots.
