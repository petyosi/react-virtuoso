...
A table can be in three "nothing to show right now" situations: it has zero rows to display, it's loading rows that haven't arrived yet, or a load failed. Each one has its own slot, and the shadcn wrapper ships sensible defaults for all of them — override a slot when the default doesn't say what your users need to hear.
