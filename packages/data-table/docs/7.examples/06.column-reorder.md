...
Each header carries a grip and a drop zone. Drag a grip over another header to move the source column before or after the target.

The shadcn components import from `@virtuoso.dev/data-table/column-reorder` — consumers that never reorder columns don't ship that reducer.
