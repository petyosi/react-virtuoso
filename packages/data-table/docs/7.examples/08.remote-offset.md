---
title: Load Server-side Paged Data
description: Fetch only the visible slice of rows for a large dataset while preserving filtering and total counts.
sidebar:
  label: Server-side Pages
---

Offset mode is a natural fit for classic paged backends and any API that can answer "rows N through N + limit".

Use this pattern when the backend can return a total row count and an arbitrary slice. The table can
keep the full scroll height stable with placeholders while requesting only the pages near the
viewport.
