...
When a user resizes a column, hides another, drags one to a new position, and applies a filter, they expect their setup to still be there after a reload. `DataTableStatePersistence` saves column width, order, visibility, and model action state under a storage key so the table comes back the way the user left it. Two requirements: mount the component inside the table, and pass an adapter for each feature you want persisted.
