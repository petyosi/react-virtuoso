...
Column groups are a header structure feature. Use them when adjacent columns form one concept, such
as inventory metrics, fulfillment details, or financial values. Wrap related `DataTableColumn`
declarations in `ColumnGroup` and provide a `ColumnGroupHeader`.
...
Column groups affect the header layout, not the row data. The columns inside the group still keep
their own fields, cells, widths, visibility, and persistence state.

Column groups can be combined with sticky columns and column reordering. For group-level drag handles,
use `DraggableGroupHeader` from the shadcn column-reorder registry component. Keep group labels short
because they add vertical header space.

See [Banded Column Headers](/data-table/examples/column-groups/) for a complete grouped-header
