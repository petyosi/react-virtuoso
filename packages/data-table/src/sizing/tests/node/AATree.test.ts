...
function shuffle(array: number[], random: () => number) {
  let index = -1
...
const RANGE_END = 100
const RANDOM_SEQUENCE_SEED = 1337
const REMOVAL_SEQUENCE_SEED = 9001
...
describe('AATree', () => {
  it('starts with an empty tree', () => {
    ...
    expect(AA.keys(tree)).toStrictEqual([])
  ...
  it('preserves increasing sequence', () => {
    ...
    expect(isInvariant(tree)).toBe(true)
    expect(AA.keys(tree)).toStrictEqual(numbers)
  ...
  it('preserves random sequence', () => {
    const numbers = range(0, RANGE_END)
    const tree = numbersToAATree(shuffle(numbers, mulberry32(RANDOM_SEQUENCE_SEED)))
    expect(isInvariant(tree)).toBe(true)
    expect(AA.keys(tree)).toStrictEqual(numbers)
    keyMatchesValues(numbers, tree)
  })
  ...
  it('remains invariant after removal', () => {
    const numbers = range(0, RANGE_END)
    const tree = numbersToAATree(shuffle(numbers, mulberry32(REMOVAL_SEQUENCE_SEED)))
    const [evens, odds] = partition(numbers, (x) => x % 2 === 0)

    const trimmedAATree = odds.reduce((acc, n) => {
      return AA.remove(acc, n)
    }, tree)
    expect(AA.keys(trimmedAATree)).toStrictEqual(evens)
    expect(isInvariant(trimmedAATree)).toBe(true)

    const emptyAATree = evens.reduce((acc, n) => {
      return AA.remove(acc, n)
    }, trimmedAATree)

    expect(isInvariant(emptyAATree)).toBe(true)
    expect(AA.keys(emptyAATree)).toStrictEqual([])
  })

  it('produces ranges for a given range', () => {
    const tree = numbersToAATree([0, 4, 10, 15, 20])

    expect(AA.rangesWithin(tree, 3, 22)).toStrictEqual([
  ...
  it('produces ranges for a collapsed range', () => {
    ...
    expect(AA.rangesWithin(tree, 3, 3)).toStrictEqual([{ end: Number.POSITIVE_INFINITY, start: 3, value: 3 }])
  })

  it('finds the largest number that does not exceed a given value', () => {
    const tree = numbersToAATree([0, 1, 2, 3, 4, 7, 20])
    expect(AA.findMaxKeyValue(tree, 3)[0]).toBe(3)
    expect(AA.findMaxKeyValue(tree, 12)[0]).toBe(7)
    expect(AA.findMaxKeyValue(tree, 100)[0]).toBe(20)
  })
  ...
  it('can modify items in the tree', () => {
    ...
    expect(AA.walk(updatedTree)).toStrictEqual([
      { k: 0, v: 0 },
      { k: 4, v: 4 },
      { k: 20, v: 10 },
      { k: 25, v: 15 },
