...
export function insertRanges(sizeTree: AANode, ranges: SizeRange[], groupIndices?: Set<number>) {
  ...
  for (const range of ranges) {
    ...
    for (const { start: rangeStart, end: rangeEnd, value: rangeValue } of overlappingRanges) {
      ...
      // next range
      if (rangeEnd > endIndex && endIndex >= rangeStart && rangeValue !== size && !groupIndices?.has(rangeStart)) {
