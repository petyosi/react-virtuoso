...
import { type Emitter, type StatefulStream, type Stream, type Subscription, type Unsubscribe, connect, subscribe } from './actions'
...
export function eventHandler<T>(emitter: Emitter<T>) {
  ...
  return ((action: SUBSCRIBE | RESET, subscription?: Subscription<T>) => {
    switch (action) {
      case SUBSCRIBE: {
        if (subscription) {
