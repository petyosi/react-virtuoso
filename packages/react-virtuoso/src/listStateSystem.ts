import { type Range, empty, findMaxKeyValue, rangesWithin } from './AATree'
...
import { type Data, type SizeState, hasGroups, originalIndexFromItemIndex, rangesWithinOffsets, sizeSystem } from './sizeSystem'
...
function probeItemSet(index: number, sizes: SizeState, data: Data) {
  if (hasGroups(sizes)) {
    ...
    return [
      ...
      { index: itemIndex, size: 0, offset: 0, data: data?.[0] },
    ]
  }
  return [{ index, size: 0, offset: 0, data: data?.[0] }]
}
            ...
            if (topItemsIndexes.length > 0) {
              ...
              for (const range of rangesWithin(sizeTree, startIndex, endIndex)) {
                ...
                for (let i = rangeStartIndex; i <= rangeEndIndex; i++) {
                  topItems.push({ index: i, size, offset: offset, data: data?.[i] })
                  offset += size
                }
              }
            }
            ...
            const items = u.tap([] as Item<any>[], (result) => {
              for (const range of offsetPointRanges) {
                ...
                for (let i = rangeStartIndex; i <= endIndex; i++) {
                  ...
                  result.push({ index: i, size, offset: offset, data: data?.[i] })
                  offset += size + gap
                }
              }
            })
