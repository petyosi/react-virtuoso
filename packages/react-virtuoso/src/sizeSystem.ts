import { type AANode, type Range, arrayToRanges, empty, find, findMaxKeyValue, insert, newTree, rangesWithin, remove, walk } from './AATree'
import type { SizeFunction, SizeRange } from './interfaces'
import { type Log, LogLevel, loggerSystem } from './loggerSystem'
...
export function insertRanges(sizeTree: AANode<number>, ranges: SizeRange[]) {
  let syncStart = empty(sizeTree) ? 0 : Number.POSITIVE_INFINITY
...
function createOffsetTree(prevOffsetTree: OffsetPoint[], syncStart: number, sizeTree: AANode<number>, gap: number) {
  ...
  for (const { start: startIndex, value } of rangesWithin(sizeTree, syncStart, Number.POSITIVE_INFINITY)) {
...
export function sizeTreeToRanges(sizeTree: AANode<number>): SizeRange[] {
  return walk(sizeTree).map(({ k: startIndex, v: size }, index, sizeArray) => {
    ...
    const endIndex = nextSize ? nextSize.k - 1 : Number.POSITIVE_INFINITY
          ...
          if (groupedMode) {
            ...
            while (prependedGroupItemsCount < unshiftWith) {
              ...
              const groupItemCount =
                sizes.groupIndices.length === groupIndex + 1
                  ? Number.POSITIVE_INFINITY
                  : sizes.groupIndices[groupIndex + 1] - theGroupIndex - 1
