...
- `listOffset` - the distance between the list top and viewport top.
- `visibleListHeight` - the visible list height without headers and footers.
- `scrollHeight` - the scroll element's total height.
- `bottomOffset` - the distance from the scroll element bottom to the viewport bottom.
- `isAtBottom` - whether the list is at the bottom or currently scrolling there.
- `lastVisibleItemIndex` - the index of the bottom-most visible item.
- `lastItemBottomOffset` - the distance from that item's bottom edge to the viewport bottom.
