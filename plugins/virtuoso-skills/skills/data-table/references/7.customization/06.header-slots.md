...
Slot UI must be mounted through its slot component, not rendered as ordinary header content. For example, `SortHeaderButton` belongs in `HeaderEnd`:
...
That bypasses the header-slot layout and can render the sort controls as a separate header row while the table falls back to raw field labels.
