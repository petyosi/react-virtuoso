...
- `ResizeHandle` from `@/components/ui/data-table/column-resize` — mounts in `HeaderEdge`. Install with registry item `data-table-resize-handle`. See [Column Resizing](/data-table/columns/column-resizing/).
...
- `SortHeaderButton` from `@/components/ui/data-table/column-sort` — mounts in `HeaderEnd` and dispatches a model `sort` action. Install with registry item `data-table-sort-header-button`. See [Remote Column Sorting](/data-table/examples/remote-column-sorting/).
